...
export default function createAdminUsersRouter(supabase, adminSupabase) {
  ...
  router.post('/', async (req, res) => {
    try {
      ...
      console.log('🔍 DEBUG CREATE USER (server) - Request body:', req.body);
      console.log('🔍 DEBUG CREATE USER (server) - phone_number extracted:', phone_number);
      ...
      console.log('🔍 DEBUG CREATE USER (server) - Data sent to Supabase:', {
        id: authUser.user.id,
        name,
        email,
        phone_number: phone_number,
      ...
      console.log('🔍 DEBUG CREATE USER (server) - Supabase response:', data);
      console.log('🔍 DEBUG CREATE USER (server) - Supabase error:', error);
      if (error) {
  ...
  router.put('/:id', async (req, res) => {
    ...
    console.log('🔍 DEBUG UPDATE USER (server) - Request body:', req.body);
    console.log('🔍 DEBUG UPDATE USER (server) - phone_number extracted:', phone_number);
    ...
    try {
      ...
      console.log('🔍 DEBUG UPDATE USER (server) - Data sent to Supabase:', {
        name,
        email,
        phone_number: phone_number,
        role,
      ...
      console.log('🔍 DEBUG UPDATE USER (server) - Supabase response:', data);
      console.log('🔍 DEBUG UPDATE USER (server) - Supabase error:', error);
      if (error) {
