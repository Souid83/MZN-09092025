...
const Transport = () => {
  ...
  const [filteredSlips, setFilteredSlips] = useState<TransportSlip[]>([]);
  ...
  const checkInvoiceStatusesForSlips = async () => {
    ...
    for (const slip of slips) {
      if (slip.status === 'delivered') {
        try {
          const hasInvoice = await checkInvoiceExists(slip.id, 'transport');
  ...
  const applyFilters = () => {
    ...
    if (dateFilter.start) {
      filtered = filtered.filter(slip => 
        new Date(slip.delivery_date) >= new Date(dateFilter.start)
      );
    }
    if (dateFilter.end) {
      filtered = filtered.filter(slip => 
        new Date(slip.delivery_date) <= new Date(dateFilter.end)
      );
    }
  ...
  const handleGenerateMultipleInvoices = async () => {
    ...
    try {
      ...
      await createGroupedInvoice(slipsToInvoice, 'transport');
  ...
  const handleGenerateInvoice = async (slip: TransportSlip) => {
    try {
      await createInvoiceFromSlip(slip, 'transport');
