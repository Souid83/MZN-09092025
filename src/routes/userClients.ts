...
router.get('/user-clients/:userId', requireAuth, [
  ...
  try {
    ...
    // Clients attributed to the user
    ...
    if (attributionsError) {
      console.error('[user-clients] attributionsError:', attributionsError);
    }
    if (!attributionsError && userAttributions) attributions = userAttributions;
